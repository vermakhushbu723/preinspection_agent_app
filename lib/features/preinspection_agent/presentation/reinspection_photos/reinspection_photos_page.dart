...
class ReinspectionPhotosPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ReinspectionPhotosPage> createState() =>
      _ReinspectionPhotosPageState();
}

class _ReinspectionPhotosPageState
    extends ConsumerState<ReinspectionPhotosPage> {
  ...
  Future<void> _addExtraPhoto() async {
    ...
    final saved = await _mediaStorage.savePhoto(
      'reinspection_extra_${DateTime.now().millisecondsSinceEpoch}',
      file.path,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final uploadedCount =
        (_mainPhotoPath != null ? 1 : 0) +
        flow.reinspectionPhotos.length +
        (_billPath != null ? 1 : 0);
                        ...
                        children: [
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () =>
                                  _pickMainPhoto(ImageSource.camera),
                          ...
                          Expanded(
                            child: OutlinedButton(
                              onPressed: () =>
                                  _pickMainPhoto(ImageSource.gallery),
